...
LMDBCursor* LMDB::NewCursor() {
  ...
  return new LMDBCursor(mdb_txn, mdb_cursor, mdb_stat.ms_entries);
}
...
// lingni implement shuffle LMDB
void LMDBCursor::readAllKeys()
{
  if (all_keys_.size())
  {
    LOG(INFO) << "key values are already set up, entries = " << all_keys_.size();
    return;
  }
  LOG(INFO) << "LMDB stored entries count = " << nr_entries_;
  all_keys_.clear();

  // try finding all the keys present
  int mdb_status = mdb_cursor_get(mdb_cursor_, &mdb_key_, &mdb_value_, MDB_FIRST);
  CHECK_NE(mdb_status, MDB_NOTFOUND) << "fail to find the first entry";
  std::string kcurr = key();
  all_keys_.push_back(kcurr);
  DLOG(INFO) << "entry 0: " << kcurr;
  for(int i =1; i < nr_entries_; ++i)
  {
    mdb_status = mdb_cursor_get(mdb_cursor_, &mdb_key_, &mdb_value_, MDB_NEXT);
    CHECK_NE (mdb_status, MDB_NOTFOUND) << "fail to find entry " << i;
    kcurr = key();
    DLOG(INFO) << "entry " << i <<": " << kcurr;
    all_keys_.push_back(kcurr);
  }

  CHECK_EQ(nr_entries_, all_keys_.size()) << "nr_entries = " << nr_entries_ << ", but found keys = "<< all_keys_.size();
}
